...
class FakeContainer:
    def __init__(
        self,
        *,
        name: str = "opsguard-demo-service",
        managed: bool = True,
        running: bool = True,
        version: str = "v1-stable",
        startup_fault: str = "none",
        image: str = "opsguard-demo:local",
    ) -> None:
        self.name = name
        self.restart_calls = 0
        self.stop_calls = 0
        self.remove_calls = 0
        labels = {"com.opsguard.managed": "true"} if managed else {}
        self.attrs = {
            "Id": "abcdef1234567890",
            "State": {
...
def _settings() -> Settings:
    return Settings(
        ...
        docker_container_name="opsguard-demo-service",
        docker_demo_service_url="http://opsguard-demo-service:8081",
        docker_network_name="opsguard-network",
        docker_network_alias="demo-service",
        docker_demo_image="opsguard-demo:local",
    )
...
def test_rollback_recreates_the_container_with_stable_version(monkeypatch):
    ...
    assert call["network"] == "opsguard-network"
